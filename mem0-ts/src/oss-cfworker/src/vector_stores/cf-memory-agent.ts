...
export class CfMemoryAgent extends Agent<Env> {
	...
	private async getOne(sql: TemplateStringsArray | string, params: (string | number | boolean | null)[] = []): Promise<any> {
		return new Promise((resolve, reject) => {
			try {
				const row = this.sql(sql as TemplateStringsArray, ...params);
				resolve(row);
			} catch (e) {
				reject(e);
			}
	...
	async insert(
		...
		for (let i = 0; i < vectors.length; i++) {
			...
			await this.run(
				...
				[ids[i], vectorText, JSON.stringify(payloads[i])],
			);
		}
	}
	...
	async update(
		...
		await this.run(
			`UPDATE vectors SET vector = ?, payload = ? WHERE id = ?`,
			[vectorText, JSON.stringify(payload), vectorId],
		);
	}
